...
public record DropShadowBorder(Color lineColor, int lineWidth, int shadowSize, float shadowOpacity, int cornerSize,
    ...
    public void paintBorder(Component c, Graphics graphics, int x, int y, int width, int height) {
         ...
         */
        var images = getImages((Graphics2D) graphics);
...
//        int bottomEdge = y + height - borderInsets.bottom;
    ...
    private Map<Position, BufferedImage> getImages(Graphics2D g2) {
        ...
        var images = CACHE.get(shadowSize);
        if (images == null) {
            images = new HashMap<>();
             ...
             *     drawing the Border
             */
            var rectWidth = cornerSize + 1;
            RoundRectangle2D rect = new RoundRectangle2D.Double(0, 0, rectWidth, rectWidth, cornerSize, cornerSize);
            var imageWidth = rectWidth + shadowSize * 2;
            var image = GraphicsUtilities.createCompatibleTranslucentImage(imageWidth, imageWidth);
            var buffer = (Graphics2D) image.getGraphics();
            buffer.setColor(new Color(0.0f, 0.0f, 0.0f, shadowOpacity));
            buffer.translate(shadowSize, shadowSize);
            buffer.fill(rect);
            buffer.dispose();

            var blurry = 1.0f / (float) (shadowSize * shadowSize);
            var blurKernel = new float[shadowSize * shadowSize];
            Arrays.fill(blurKernel, blurry);
            var blur = new ConvolveOp(new Kernel(shadowSize, shadowSize, blurKernel));
            var targetImage = GraphicsUtilities.createCompatibleTranslucentImage(imageWidth, imageWidth);
    ...
    public Insets getBorderInsets(Component c) {
        ...
        var bottom = showBottomShadow ? lineWidth + shadowSize : lineWidth;
        var right = showRightShadow ? lineWidth + shadowSize : lineWidth;
